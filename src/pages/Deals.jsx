...
export default function Deals() {
  ...
  const listings     = deals.filter(d=>d.deal_type==='listing')
  const wholesales   = deals.filter(d=>d.deal_type==='wholesale')
    ...
    <PageWrap pad={!mobile}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        <div>
          ...
          <p style={{ fontSize:12, color:'#6b7280', marginTop:2 }}>NHC listings, wholesales and referrals — properties we don't purchase</p>
        </div>
        <Btn onClick={()=>setDrawer({...EMPTY})}>+ Add Deal</Btn>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns: mobile ? 'repeat(2,1fr)' : 'repeat(5,1fr)', gap:12, marginBottom:20 }}>
        ...
        <StatCard label="NHC Commission" value={fmtK(totalComm)} sub="closed deals" topColor="#3B6D11" />
        <StatCard label="Closed Volume" value={fmtK(totalVolume)} sub="sale prices" topColor="#B8892A" />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : 'repeat(3,1fr)', gap:10, marginBottom:20 }}>
        {[['listing','Listings',listings.length,'#3B6D11'],['wholesale','Wholesales',wholesales.length,'#6b21a8'],['referral','Referrals',deals.filter(d=>d.deal_type==='referral').length,'#2D6FAF']].map(([t,l,c,color])=>(
          <div key={t} onClick={()=>setTypeFilter(typeFilter===t?'all':t)} style={{ background:'#fff', borderRadius:8, border:`2px solid ${typeFilter===t?color:'#D6D2CA'}`, padding:'10px 14px', cursor:'pointer', transition:'all 0.15s' }}>
            ...
            <div style={{ fontSize:22, fontWeight:700, color:typeFilter===t?color:'#2C2C2C' }}>{c}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:14 }}>
        {['all','active_listing','pending','closed','lost'].map(s=>(
          <button key={s} onClick={()=>setStatusFilter(s)} style={{ padding:'5px 14px', border:'none', borderRadius:4, cursor:'pointer', background:statusFilter===s?'#2C2C2C':'#F0EDE6', color:statusFilter===s?'#fff':'#6b7280', fontSize:11, fontWeight:statusFilter===s?700:400, fontFamily:'inherit' }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth: mobile ? 0 : 600 }}>
            ...
            {closed.length>0 && (
              <tfoot><tr style={{ borderTop:'2px solid #D6D2CA', background:'#F0EDE6' }}>
                <td colSpan={3} style={{ padding:'8px 14px', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:0.8 }}>Closed Totals</td>
...
function DealDrawer({ deal, mailings, open, onClose, onSave }) {
  ...
  const typeBtn = (val,color) => ({ flex:1, padding:'8px', border:'none', borderRadius:6, cursor:'pointer', fontWeight:form.deal_type===val?700:400, fontSize:12, fontFamily:'inherit', background:form.deal_type===val?color:'#F0EDE6', color:form.deal_type===val?'#fff':'#6b7280', textTransform:'capitalize' })
    ...
    <Drawer open={open} onClose={onClose} title={form.address||'New Deal'} subtitle={form.deal_type ? `${form.deal_type} · ${STATUS_LABELS[form.status]||form.status}` : ''}>
      <div style={{ display:'flex', flexDirection:'column', gap:14, paddingTop:12 }}>
        ...
        <Field label="Commission Earned ($)">
          <input style={monoInp} type="number" value={form.commission_earned||''} onChange={set('commission_earned')} placeholder="Auto-calculated" />
        </Field>
        ...
        <Field label="Notes"><textarea style={{ ...inp, minHeight:72, resize:'vertical' }} value={form.notes||''} onChange={set('notes')} placeholder="Agent, seller situation, deal details..." /></Field>

        <div style={{ display:'flex', justifyContent:'space-between', marginTop:8 }}>
          {!isNew && <Btn variant="danger" onClick={del}>Delete</Btn>}
          <div style={{ display:'flex', gap:8, marginLeft:'auto' }}>
            <Btn variant="outline" onClick={onClose}>Cancel</Btn>
            <Btn onClick={save} disabled={!form.address}>Save Deal</Btn>
          </div>
        </div>
      </div>
    </Drawer>
