...
const EMPTY_MAILING = { campaign_name: '', drop_date: '', list_size: '', piece_type: 'Postcard', notes: '', entity: 'NHC' }
...
export default function MailingTracker() {
  ...
  const [tab, setTab] = useState('campaigns') // campaigns | deals | analytics
  ...
  const convRate = totalSent > 0 ? ((closedDeals.length / totalSent) * 100).toFixed(2) : 0
  ...
  return (
    <PageWrap>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        ...
        <StatCard label="Closed Deals" value={closedDeals.length} topColor="#3B6D11" />
        <StatCard label="Commission Earned" value={fmtK(totalComm)} sub={`${convRate}% conv rate`} topColor="#3B6D11" />
      </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#F0EDE6' }}>
                    {['Campaign', 'Drop Date', 'List Size', 'Piece Type', 'Deals Sourced', 'Actions'].map(h => (
      ...
      {tab === 'deals' && (
        <>
          ...
          {deals.length === 0 ? <EmptyState icon="○" text="No deals yet. Add a deal to track it." /> : (
            <Card style={{ padding: 0 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#F0EDE6' }}>
                    {['Address', 'Type', 'Status', 'Sale Price', 'Commission', 'Source', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '8px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, letterSpacing: 0.8, color: '#6b7280', textTransform: 'uppercase' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {deals.map((d, i) => {
                    ...
                    return (
                      <tr key={d.id} style={{ background: i % 2 === 0 ? '#fff' : '#FAFAF8', borderTop: '0.5px solid #F0EDE6' }}>
                        ...
                        <td style={{ padding: '10px 16px', fontSize: 12 }}><Badge color={d.deal_type === 'listing' ? '#3B6D11' : '#D97825'}>{d.deal_type}</Badge></td>
      ...
      {mailingModal && (
        <MailingForm
          mailing={mailingModal}
          onClose={() => setMailingModal(null)}
          onSave={() => { setMailingModal(null); load() }}
        />
      )}
...
function MailingForm({ mailing, onClose, onSave }) {
  ...
  async function save() {
    const payload = { campaign_name: form.campaign_name, drop_date: form.drop_date || null, list_size: parseInt(form.list_size) || null, piece_type: form.piece_type, notes: form.notes, entity: form.entity || 'NHC' }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {form.is_flip && (
          <div style={{ background: '#f0f6ff', borderRadius: 8, padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <FieldRow>
              <Field label="Purchase Cost ($)"><input style={monoInp} type="number" value={form.purchase_price || ''} onChange={set('purchase_price')} placeholder="120000" /></Field>
